...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record probeRecord : probeRecords) {
            ...
            for (Record lRecord : hashTable.get(probeJoinValue)) {
                Record joinRecord = lRecord.concat(probeRecord);
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        //最多传5次 maxPagesNum = (B-1)^5*(B-2) = 5^5*4 = 12500
        //maxRecordsNum = 8*12500 = 100,000
        //但这样的思路现实中行不通了,换个角度：最多4个page 最多32条record, 我们给33个一样的值即可
        for (int i = 0 ; i < 33; i++) {
